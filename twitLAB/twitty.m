...
% 5. Setting up a proper SSL certificate.
%
% ============
% 1. Overview
% ============
...
% The former retrieves random samples of all public statuses, whereas the later method allows to
...
% interrupt the connection, a user has to press <CTRL>-C. 
...
% 
% NB: The sampleSize and batchSize (see below) properties do not have any effect on the methods using
% the Search API. 
%
% -----------------
% OUTPUT FUNCTIONS
...
% 
% An output function can be a usual m-file, e.g., the "tweetingSummary.m" file provided with the
...
% corresponidng to a valid user account. The credentials consist of four strings: Consumer (API) key,
% Consumer (API) secret, Access token, and Access token secret. The former two correspond to the
...
%    apps.twitter.com, or hovering over your profile image in the top right hand corner of
...
%    Upon creation, your application will be assigned the two Consumer (API) keys.
%
...
%    This can be done either at the constructor call, i.e., tw = twitty(credentials),
...
% 5. Setting up a proper SSL certificate
% ======================================
% Twitter uses a secure SSL connection to communicate with it's clients.
% For details, see https://dev.twitter.com/docs/security/using-ssl
% 
% To establish a secure connection, twitty must have a proper root
% certificate installed in its java keystore of trusted CA (certification
% authorities). api.twitter.com is signed by "VeriSign Class 3 Public
% Primary Certification Authority - G5" root certificate.
% 
% In a nutshell, from a user perspective a root certificate is a file: 
% "VeriSign Class 3 Public Primary Certification Authority - G5.cer"
% It can be downloaded (among other certificates) at
% http://www.symantec.com/page.jsp?id=roots 
% Download the roots.zip. The required file is in the "Generation 5 (G5)
% PCA" folder. 
% To import the certificate, i.e., include it in the JAVA keystore of
% trusted certificates, you can try the importcert.m function available
% here: http://www.mathworks.com/matlabcentral/answers/uploaded_files/1142/importcert.m  
% Alternatively, you can use the "keytool.exe" (assuming you are under
% Windows) tool directly: 
% 1. Navigate to something like: 
%  C:\Program Files\MATLAB\R2013a\sys\java\jre\win64\jre\bin 
% 2. Run: keytool -import -trustcacerts -file "C:\tmp\VeriSign Root
% Certificates\Generation 5 (G5) PCA\VeriSign Class 3 Public Primary
% Certification Authority - G5.cer" -alias verisignclass3g5ca -keystore
% ../lib/security/cacerts -storepass changeit
% 
% Substitute the proper path to the certificate file, of course.
% That's it. Now twitty should be able to connect to the Twitter API.

% v1.1.2 (c) 2012-14, Vladimir Bondarenko <http://sites.google.com/site/bondsite> 
...
%% Streaming
        ...
        % (Replaces the publicTimeline() call in the API v1.0)
        %
...
%% Accounts
        ...
        % tw = twitty; S = tw.accountVerifyCredentials('include_entities','true');
            ...
            S = twtr.callTwitterAPI(httpMethod,url,params,1);
            if any(strcmpi('errors',fieldnames(S{1})))
                warning('Twitter responded with an error:');
                disp(S{1}.errors{1})
            end
...
%% The main API caller
    methods(Access = private)
        ...
        % (c) 2012-14, Vladimir Bondarenko <http://sites.google.com/site/bondsite>
...
%                     theURL = URL([url '?' paramStr]);
        ...
        try
            ...
            warning('twitty:connectionError', ME.message);
            inStream = BufferedReader( InputStreamReader(httpConn.getErrorStream) );
        end
